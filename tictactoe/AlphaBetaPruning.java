...
public class AlphaBetaPruning {
	...
	public static double maxValue(GameState s, double alpha, double beta) {
		if(s.isTerminalState()){return s.value();}
		
		if(s.whoseTurn() == 0) {
			for(GameState i: s.getSuccessors())
			{
					
				alpha = Math.max(alpha, minValue(i, alpha, beta));
	...
	public static double minValue(GameState s, double alpha, double beta) {
		...
		for(GameState i : s.getSuccessors())
		{
			beta = Math.min(beta, maxValue(i, alpha, beta));
	...
	public static double maxValue(GameState s, double alpha, double beta, int searchDepth, GameHeuristic h) {
		if(s.isTerminalState() || searchDepth == 0)
		{
			return h.h(s) ; 
		}
		if(s.whoseTurn() == 0) {
			ArrayList<GameState> successors = s.getSuccessors();
			for (int i = 0; i < successors.size(); i++) 

			{
					
				alpha = Math.max(alpha, minValue(successors.get(i), alpha, beta, searchDepth-1, h));
				if(alpha >= beta) {return beta;}
					
			}
		}
		else {
			return minValue(s, alpha, beta, searchDepth-1, h);
		}
	...
	public static double minValue(GameState s, double alpha, double beta, int searchDepth, GameHeuristic h) {
		...
		if(s.whoseTurn() == 1) {
			...
			for (int i = 0; i < successors.size(); i++) {
				...
				beta = Math.min(beta, maxValue(successors.get(i), alpha, beta, searchDepth-1, h));
				if(beta <= alpha) {return alpha;}
						
			}
			}
			else {
				return maxValue(s, alpha, beta, searchDepth-1, h);
			}
		return beta;
	}
}
